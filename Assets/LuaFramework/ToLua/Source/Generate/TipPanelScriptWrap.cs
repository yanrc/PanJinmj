//this source code was auto-generated by tolua#, do not modify it
using System;
using LuaInterface;

public class TipPanelScriptWrap
{
	public static void Register(LuaState L)
	{
		L.BeginClass(typeof(TipPanelScript), typeof(UnityEngine.MonoBehaviour));
		L.RegFunction("setText", setText);
		L.RegFunction("startAction", startAction);
		L.RegFunction("TipsMoveCompleted", TipsMoveCompleted);
		L.RegFunction("__eq", op_Equality);
		L.RegFunction("__tostring", ToLua.op_ToString);
		L.RegVar("label", get_label, set_label);
		L.EndClass();
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int setText(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			TipPanelScript obj = (TipPanelScript)ToLua.CheckObject<TipPanelScript>(L, 1);
			string arg0 = ToLua.CheckString(L, 2);
			obj.setText(arg0);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int startAction(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			TipPanelScript obj = (TipPanelScript)ToLua.CheckObject<TipPanelScript>(L, 1);
			float arg0 = (float)LuaDLL.luaL_checknumber(L, 2);
			obj.startAction(arg0);
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int TipsMoveCompleted(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 1);
			TipPanelScript obj = (TipPanelScript)ToLua.CheckObject<TipPanelScript>(L, 1);
			obj.TipsMoveCompleted();
			return 0;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int op_Equality(IntPtr L)
	{
		try
		{
			ToLua.CheckArgsCount(L, 2);
			UnityEngine.Object arg0 = (UnityEngine.Object)ToLua.ToObject(L, 1);
			UnityEngine.Object arg1 = (UnityEngine.Object)ToLua.ToObject(L, 2);
			bool o = arg0 == arg1;
			LuaDLL.lua_pushboolean(L, o);
			return 1;
		}
		catch (Exception e)
		{
			return LuaDLL.toluaL_exception(L, e);
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int get_label(IntPtr L)
	{
		object o = null;

		try
		{
			o = ToLua.ToObject(L, 1);
			TipPanelScript obj = (TipPanelScript)o;
			UnityEngine.UI.Text ret = obj.label;
			ToLua.Push(L, ret);
			return 1;
		}
		catch(Exception e)
		{
			return LuaDLL.toluaL_exception(L, e, o, "attempt to index label on a nil value");
		}
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int set_label(IntPtr L)
	{
		object o = null;

		try
		{
			o = ToLua.ToObject(L, 1);
			TipPanelScript obj = (TipPanelScript)o;
			UnityEngine.UI.Text arg0 = (UnityEngine.UI.Text)ToLua.CheckObject<UnityEngine.UI.Text>(L, 2);
			obj.label = arg0;
			return 0;
		}
		catch(Exception e)
		{
			return LuaDLL.toluaL_exception(L, e, o, "attempt to index label on a nil value");
		}
	}
}

