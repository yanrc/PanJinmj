...
function RuleSelect.CreateItem(ruletable, parent)
	...
	for i = 1, #ruletable.groups do
		...
		local panel = parent:Find(item.title);

		if (panel == nil) then
			panel = newObject(Group, parent).transform;				
			-- 标题
			local Text = panel:Find("Text"):GetComponent("Text");
			Text.text = item.title;
		else
			-- 有时候同一标签下有多个组
			panel = newObject(Group, parent).transform;
			-- 标题
			panel:Find("Text").gameObject:SetActive(false)
		end
			panel.anchoredPosition = Vector2.New(item.pos[1], item.pos[2])
			panel.gameObject:SetActive(true);
			panel.name = item.title;
			root = panel:Find("root");
		...
		for j = 1, #item.content do
			...
			local main = obj.transform:Find("Label"):GetComponent("Text")
			local des = obj.transform:Find("lbDes"):GetComponent("Text")
			-- 消耗房卡数
			local cost = obj.transform:Find("cost/Text"):GetComponent("Text")
			local image = obj.transform:Find("cost/Image"):GetComponent("Image")
