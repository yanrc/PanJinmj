...
function RecordItem.New(go, lua)
	...
	recordItem.lbIndex = go.transform:Find('Texts/index'):GetComponent('Text')
	recordItem.lbRoomid = go.transform:Find('Texts/roomid/Text'):GetComponent('Text')
	recordItem.lbTime = go.transform:Find('Texts/time/Text'):GetComponent('Text')
	for i = 1, 4 do
		recordItem.lbNames[i] = go.transform:Find('Texts/player' .. i):GetComponent('Text')
		recordItem.lbScores[i] = go.transform:Find('Texts/player' .. i .. '/Text'):GetComponent('Text')
	end
	recordItem.btnShowDetail = go.transform:Find('btnShowDetail').gameObject
