...
function RecordPanel.OnCreate(obj)
	...
	this:Init(obj)
	RecordPage = transform:Find('RecordPage').gameObject
	DetailPage = transform:Find('DetailPage').gameObject
	btnClose = transform:Find('btnClose').gameObject
	btnWatch = transform:Find('btnWatch').gameObject
	InputPanelbtnConfirm = transform:Find('InputPanel/btnConfirm').gameObject
	InputPanelbtnCancel = transform:Find('InputPanel/btnCancel').gameObject
	inputField = transform:Find('InputPanel/InputField'):GetComponent('InputField')
	InputPanel = transform:Find('InputPanel').gameObject
	recordItemsRoot = transform:Find('RecordPage/scrollrect/root')
	detailItemsRoot = transform:Find('DetailPage/scrollrect/root')
	for i = 1, 4 do
		playerText[i] = transform:Find('DetailPage/Header/root/Player' .. i):GetComponent('Text')
	end
