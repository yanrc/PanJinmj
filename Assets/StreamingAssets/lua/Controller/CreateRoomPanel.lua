...
function CreateRoomPanel.OnCreate(obj)
	...
	this:Init(obj)
	-- panelDevoloping = transform:Find("root/content/Panel_Developing").gameObject
	local createButton = transform:Find("root/content/Button_Create_Sure").gameObject
	local cancelButton = transform:Find("root/Image_Create_Bg/Button_Delete").gameObject
	local tab = transform:Find("root/Panel_Game_Buttons/Button_base").gameObject
	local tabsRoot = transform:Find("root/Panel_Game_Buttons")
	...
	RuleSelectObj = transform:Find("root/content/Panel_Game_Setting").gameObject
	RuleSelect.Init(RuleSelectObj)
	for i = 1, #APIS.Rules do
		local obj = newObject(tab, tabsRoot)
		obj:SetActive(true)
		local ruletable = APIS.Rules[i]
		obj.transform:Find("Text"):GetComponent("Text").text = ruletable.name
...
function CreateRoomPanel.CreateShuangliaoRoom()
	soundMgr:playSoundByActionButton(1);
	local rule, num = RuleSelect.Select2Int(ShuangliaoRule.name)
	local sendVo = {
		roundNumber = 1,
		menqing = false,
		siguiyi = false,
	...
	for i = 1, num do
		if ((bit.band(rule, 1)) == 1) then
			if (i == 1) then
				sendVo.baosanjia = true
			elseif (i == 2) then
				sendVo.siguiyi = false
			elseif (i == 3) then
				sendVo.siguiyi = true
			elseif (i == 4) then
				sendVo.menqing = false
			elseif (i == 5) then
				sendVo.menqing = true
			elseif (i == 6) then
				sendVo.roundNumber = 16
