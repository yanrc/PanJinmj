...
local function InitOtherCardList(initDiretion,count)
	for i = 1,count do
		local temp =resMgr:LoadPrefab('prefabs', {'Assets/Project/Prefabs/card/Bottom_B.prefab'},function() --实例化当前牌
...
local function MoPai()
	pickCardItem=resMgr:LoadPrefab('prefabs', {'Assets/Project/Prefabs/card/Bottom_B.prefab'},function()
		...
		if (pickCardItem ~= nil) then --有可能没牌了
			...
			pickCardItem.transform:SetParent(parentList[1]); --父节点
			pickCardItem.transform.localScale = Vector3.New(1.1,1.1,1);--原大小
			pickCardItem.transform.localPosition =Vector3.New(580, -292); --位置
...
local function PengCard(response)
	...
	if (curDirString == DirectionEnum.Bottom) then
		--自己碰牌
		putCardIntoMineList(putOutCardPoint)--给mineList[1]增加加一张牌
		mineList[2][putOutCardPoint] = 2;--保存碰掉的2张牌
...
function GetPaiInpeng(cardPoint,direction)
	...
	local switch=
	{
	[DirectionEnum.Bottom]=function()--自己
...
local function CardChange(obj)
	if (GlobalDataScript.isChiState) then
		...
		if (handCardCount % 3 == 2) then --这时候才能出牌
			obj.bottomScript.onSendMessage =nil;
			obj.bottomScript.reSetPoisiton =nil;
			local putOutCardPointTemp = obj.bottomScript.CardPoint;
			pushOutFromMineList(putOutCardPointTemp);                         --将打出牌移除
			table.remove(handerCardList[1],obj)
			Destroy(obj);
			SetPosition(false);
			CreatePutOutCardAndPlayAction(putOutCardPointTemp, GetMyIndexFromList(),obj.transform.position);--出牌动画
			local cardvo = {}
...
function InsertCardIntoList(item)--插入牌的方法
	if (item ~= nil) then
		local curCardPoint = item.bottomScript.CardPoint;--得到当前牌指针
		if (curCardPoint == GlobalData.roomVo.guiPai) then
			table.insert(handerCardList[1],1,item)--鬼牌放到最前面
			return;
		else
			for i = 1,#handerCardList[1] do--游戏物体个数 自增
				local cardPoint = handerCardList[1][i].bottomScript.CardPoint;--得到所有牌指针
				if (cardPoint ~= GlobalData.roomVo.guiPai and cardPoint >= curCardPoint) then --牌指针>=当前牌的时候插入
					table.insert(handerCardList[1],i,item)
					SortMyCardList();
					return;
				end
				table.insert(handerCardList[1],item)--游戏对象列表添加当前牌
				SortMyCardList();
			end
		end
		item = nil;
...
local function SetPosition(flag)--设置位置
	local count = #handerCardList[1];
	local startX = 594 - count * 80;
	if (flag) then
		for i=1,count-1 do
			handerCardList[1][i].transform.localPosition =Vector3.New(startX + i * 80, -292,0); --从左到右依次对齐
		end
		handerCardList[1][count - 1].transform.localPosition =Vector3.New(580, -292,0); --从左到右依次对齐
	else
		for i=1,count do
			handerCardList[1][i].transform.localPosition = Vector3.New(startX + i * 80 - 80, -292,0); --从左到右依次对齐
		end
	end
end
...
local function MyGangBtnClick()
	...
	--GlobalDataScript.isDrag = true;--由于存在抢杠胡，点了杠按钮以后还不能打牌，收到杠消息才能打
...
local function HupaiRequest()
	if (SelfAndOtherPutoutCard ~= -1) then
		local cardPoint = SelfAndOtherPutoutCard;--需修改成正确的胡牌cardpoint
		local requestVo = {};
...
local function OpenGameSettingDialog()
        ...
        if (canClickButtonFlag) then
            ...
            if (bankerId == GetMyIndexFromList()) then--我是房主（一开始庄家是房主）
                ss.canClickButtonFlag = canClickButtonFlag;
  ...
  --恢复其他全局数据
        ...
        for i=1, #mineList[1] do
            if (mineList[1][i] > 0) then
                for j = 1,#mineList[1][i] do
                    local gob = resMgr:LoadPrefab('prefabs', {'Assets/Project/Prefabs/card/Bottom_B.prefab'},function()
					if (gob ~= nil) then
						gob.transform:SetParent(parentList[1]);--设置父节点
						gob.transform.localScale =Vector3.New(1.1,1.1,1);
						gob.bottomScript.onSendMessage = CardChange;--发送消息fd
						gob.bottomScript.reSetPoisiton = CardSelect;
                        gob.bottomScript.CardPoint = i;
                        if (i == GlobalData.roomVo.guiPai) then
						gob.bottomScript.SetLaizi(true);
						end
						table.insert(handerCardList[1],god)--增加游戏对象
						SortMyCardList();
					end
					end)
				end
    ...
    local function DisplayPengCard()
        for i=0,#GlobalData.reEnterRoomData.playerList do
            local paiArrayType = GlobalData.reEnterRoomData.playerList[i].paiArray[2];--第二个数组存储了碰的牌
            local dirstr = GetDirection(GetIndex(GlobalDataScript.reEnterRoomData.playerList[i].account.uuid));
            if (table.Contains(paiArrayType,1)) then--1代表碰的那张牌
    ...
    local function returnGameResponse(response)
        ...
        local curAvatarIndexTemp = -1;--当前出牌人的索引
        local pickAvatarIndexTemp = -1; --当前摸牌人的索引
        local putOffCardPointTemp = -1;--当前打得点数
        local currentCardPointTemp = -1;--当前摸的点数
        --不是自己摸牌

            curAvatarIndexTemp = returnJsonData.curAvatarIndex;--当前打牌人的索引
            putOffCardPointTemp = returnJsonData.putOffCardPoint;--当前打得点数
            ...
            pickAvatarIndexTemp = returnJsonData.pickAvatarIndex;--当前摸牌牌人的索引

            if(table.Contains(returnJsonData,currentCardPoint)) then
            currentCardPointTemp = returnJsonData.currentCardPoint;--当前摸得的点数  (吃碰杠以后，服务器发的这个值是-2)
