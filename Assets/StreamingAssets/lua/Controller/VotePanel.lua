...
function VotePanel.OnCreate(obj)
	...
	this:Init(obj)
	for i = 1, 4 do
		lbNameList[i] = transform:Find('Panel/Player' .. i .. '/name'):GetComponent('Text')
		lbResult[i] = transform:Find('Panel/Player' .. i .. '/result'):GetComponent('Text')
	end
	lbTime = transform:Find('Clock/Text'):GetComponent('Text')
	btnConfirm = transform:Find('btnConfirm').gameObject
	btnCancel = transform:Find('btnCancel').gameObject
