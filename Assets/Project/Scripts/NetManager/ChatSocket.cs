    ...
    public class ChatSocket
    {
		...
		APIS.chatSocketUrl = IOSIpv6.GetIPv6Str(APIS.chatSocketUrl, APIS.chatPort.ToString(), out family);
            ...
            try
            {
                ...
                Debugger.Log("--chatSocketUrl-------开始连接-----" + APIS.chatSocketUrl + "------" + 10112);
            }
            catch (Exception ex)
            {
                //设置标志,连接服务端失败!
                ...
                Debugger.Log("--chatSocketUrl-------连接异常-----");
            }
        }
        ...
        {
            init = false;
            Debug.Log("--chatSocketUrl-------DisConnect断开连接-----");
            if (tcpclient != null)
            {
                tcpclient.Close();
                tcpclient = null;
        ...
        {
            Debugger.Log("--chatSocketUrl-------sendMsg-----sendMsg----");
            if (tcpclient != null)
            {
        ...
        {
            Debugger.Log("--chatSocketUrl-------SendData-----sendMsg----");
            try
            {
                if (stream != null)
                {
        ...
        private void ConnectCallback(IAsyncResult ar)
        {
            ...
            if ((tcpclient != null) && (tcpclient.Connected))
            {
                ...
                asyncread(tcpclient);
                Debugger.Log("--chatSocketUrl-------ConnectCallback-----聊天服务器已经连接---");
                Debugger.Log("聊天服务器已经连接!");
                //	showMessageTip ("聊天服务器已经连接!");
            }
            else
            {
                Debug.Log("--chatSocketUrl-------ConnectCallback----已经断开连接---");
                closeSocket();
                return;
            }
        ...
        private void TCPReadCallBack(IAsyncResult ar)
        {
            ...
            {
                init = false;
                Debugger.Log("--chatSocketUrl-------TCPReadCallBack-----客户端被动断开聊天服务器---");
                //被动断开时 
                mas.Close();
                state.client.Close();
                mas = null;
        ...
        private void readBuffer(BinaryReader buffers)
        {
            ...
            if (flag == 1)
            {
                ...
                Debugger.Log("--chatSocketUrl---" + "chat.headCode = " + response.headCode + "  sound.lenght =   " + soundLen);
                Debugger.Log("chat.headCode = " + response.headCode + "  sound.lenght =   " + soundLen);
